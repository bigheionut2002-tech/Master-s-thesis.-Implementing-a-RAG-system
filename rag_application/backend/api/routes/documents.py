...
from models.schemas import ChunkPoint, DocumentMetadata, VectorMapResponse
...
@router.get("/vector-map", response_model=VectorMapResponse)
def get_vector_map(
    current_user: User = Depends(get_current_user),
    document_service: DocumentService = Depends(get_document_service),
) -> VectorMapResponse:
    raw = document_service.get_vector_map(user_id=current_user.id)
    points = [
        ChunkPoint(
            id=c["id"],
            text=c["text"][:120],
    ...
    return VectorMapResponse(points=points)
